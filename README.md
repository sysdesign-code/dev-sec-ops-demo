...
We saw how we could secure the pipeline using Google Cloud's native services, such as Binary Authorization and Vulnerability scanning of the container images. We only saw one way to put some control on specific images that can be deployed to a GKE cluster. Still, Binary Authorization also offers [Build Verification](https://cloud.google.com/binary-authorization/docs/overview#attestations), in which Binary Authorization uses attestations to verify that an image was built by a specific build system or continuous integration (CI) pipeline such as Cloud Build. 
